...
\end{eqnarray*}
This is the starting point for the analysis here. The derivation
is presented in, for example, Bear and Cheng, 2010.
...
NOTE by AMP:  Looks like the density doesn't divide out the way we'd 
hoped.  Start with eqn~\ref{eq-conservation}, rewritten so that we're 
distinguishing between $\rho$, the uniform density in the cell, 
$\rho_{\sigma}$, the density associated with inflows and outflows 
along the boundary of the cell, and $\rho_{\Gamma}$, the density 
associated with other sources and sinks to and from the cell.  
Then, recognizing that $\rho$, $s$, and $\phi$ are uniform throughout 
the cell volume $V$,

\begin{equation}
  \int_V \frac{\partial (\rho s \phi)}{\partial t} \,dV +
  \oint_\sigma \rho_{\sigma} \mathbf{q} \cdot \mathbf{n} d\sigma -
  \int_V \rho_{\Gamma} \Gamma \,dV = 0,
  \label{eq-conservation-alt1}
\end{equation}

\begin{equation}
  \frac{\partial}{\partial t} \int_V \rho s \phi \,dV +
  \rho \oint_\sigma \frac{\rho_{\sigma}}{\rho} \mathbf{q} \cdot \mathbf{n} d\sigma -
  \rho \int_V \frac{\rho_{\Gamma}}{\rho} \Gamma \,dV = 0,
  \label{eq-conservation-alt2}
\end{equation}

\begin{equation}
  V \frac{\partial}{\partial t} \left ( \rho s \phi \right ) +
  \rho \oint_\sigma \frac{\rho_{\sigma}}{\rho} \mathbf{q} \cdot \mathbf{n} d\sigma -
  \rho \int_V \frac{\rho_{\Gamma}}{\rho} \Gamma \,dV = 0,
  \label{eq-conservation-alt3}
\end{equation}

\begin{equation}
  \rho s S_s V \frac{\partial h}{\partial t} +
  \rho \phi V \frac{\partial s}{\partial t} +
  \rho \oint_\sigma \frac{\rho_{\sigma}}{\rho} \mathbf{q} \cdot \mathbf{n} d\sigma -
  \rho \int_V \frac{\rho_{\Gamma}}{\rho} \Gamma \,dV = 0,
  \label{eq-conservation-alt4}
\end{equation}

\begin{equation}
  s S_s V \frac{\partial h}{\partial t} +
  \phi V \frac{\partial s}{\partial t} +
  \oint_\sigma \frac{\rho_{\sigma}}{\rho} \mathbf{q} \cdot \mathbf{n} d\sigma -
  \int_V \frac{\rho_{\Gamma}}{\rho} \Gamma \,dV = 0.
  \label{eq-conservation-alt4}
\end{equation}

\noindent The resulting balance equation shows that to conserve mass, 
we have to scale the boundary and source-sink flows by appropriate 
density ratios.  This is essentially using ``apparent volumes.''  
In other words, $\frac{\rho_{\sigma}}{\rho} \mathbf{q}$ and 
$\frac{\rho_{\sigma}}{\rho} \Gamma$ are the volumetric flow rates for 
water at density $\rho$ that correspond to the same \textit{mass} flow 
rates as the actual volumetric flow rates $\mathbf{q}$ (at density 
$\rho_{\sigma}$) and $\Gamma$  (at density $\rho_{\Gamma}$).  
Seems this concept is bound to come up (in some form) if we want 
to use a volume balance and still conserve mass in some sense.

Another way to look at it is that for eqn~\ref{eq-conservation-alt4} 
to reduce to a straight-up volume balance, we need 
$\rho_{\sigma} = \rho_{\Gamma} = \rho$.  That implies that all the 
incoming flows -- whether from other cells (or model features), or from 
external sources -- need to be at density $\rho$.  Propagating this 
requirement from cell to cell\footnote{{\color{violet}Assuming there 
are no hydraulically disconnected parts of the model.}}, we end up 
requiring that all flows throughout the model be at the same density.  
That corresponds to our ``constant-density flow'' assumption with 
variations in density being confined to specific storage, which is 
the idea behind apparent volume.
...
{\color{cyan}
REPLY by MJR: I think you are right that equation~\ref{eq-conservation}
complicates matters. The density does not divide out, as you mention,
but I think it's a result of performing the volume integral too soon here.
Let's keep working with the form from equation~\ref{eq-balance-equation}
a little bit longer. The divergence term can be expanded as
\begin{equation}
  \nabla \cdot (\rho \mathbf{q}) = 
    \mathbf{q} \cdot (\nabla \rho) + \rho \nabla \cdot \mathbf{q}
\end{equation}
and the storage term contains a component
\begin{equation}
   \frac{\partial (\rho s \phi)}{\partial t} = \dots + 
    s \phi \frac{\partial \rho}{\partial t}
\end{equation} 
It is my impression that both in literature and in practice the term
with $\nabla \rho$ is discarded assuming that the temporal
variation of $\rho$ is much greater than its spatial variation:
\begin{equation}
  \left| s \phi \frac{\partial \rho}{\partial t} \right| \gg
  \left| \mathbf{q} \cdot (\nabla \rho) \right|
  \label{eq-density-assumption}
\end{equation}
Let's assume this is valid for now, then equation~\ref{eq-balance-equation}
becomes
\begin{equation}
  \frac{\partial (\rho s \phi)}{\partial t} +
  \rho \nabla \cdot \mathbf{q} -
  \rho \Gamma = 0,
  \label{eq-balance-equation-nogradrho}
\end{equation}
If we combine this with the storage term as given by equation~\ref{eq-sto}, 
we arrive at the following macroscopic balance equation:
\begin{equation}
  ...
  \rho \Gamma = 0.
\end{equation}
Here it seems that we can safely divide out the density. We have to keep in
mind that $\Gamma$ is a volumetric source/sink of water with a density 
equal to that of the water at the location where it is applied.
In a strict sense this is not a valid mass source, but it is consistent
with how things are done in MODFLOW~6.

After some reading and thinking, I believe that I understand why
equation~\ref{eq-density-assumption} holds true, but I look forward
to discuss further. Anyway, say we pump into (or out of) an aquifer, this
will change the pressure by a significant amount. The change will propagate
through the water with the speed of sound: $1500 m/s$. This combination 
should be compared to $\mathbf{q} \nabla \rho$ which represents how much 
the density changes at a fixed point because a fluid of a different 
density is physically flowing past it. In this case the velocity is 
extremely small, namely that of Darcy flow, say $10^{-5} m/s$. And for 
the problems we are considering here, the water is assumed homogeneous 
with no variation in solute concentration or temperature. This makes 
that $\nabla \rho$ itself is negligibly small.
...
NOTE by AMP:  Thanks for your response -- it makes sense.  In my analysis,
getting to the volume balance hinges on the idea of apparent volume.
In yours, it hinges on assuming that the spatial variation is negligible
compared to the temporal variation.  But I think they're actually two sides
of the same coin -- two different ways of saying that we're going to
neglect density variations insofar as flows are concerned, but we'll
account for their effect on storage.  Your analysis makes that assumption earlier
on, before integration over the cell volume, whereas mine does so after the
integration, but i think the underlying idea is the same.  One thing to note is that
in eqns~\ref{eq-balance-equation} and \ref{eq-balance-equation-nogradrho}
I think the density in $\rho \Gamma$ needs to be interpreted as
the density of the source or sink, which for a source can be different than the
prevailing density at that location in the aquifer.  So the assumption of no
spatial variation in density needs to be extended to external sources, as well, 
which is perfectly consistent with the assumptions made in my analysis.  We
should discuss, but at this point my feeling is that we're essentially saying the
same thing, but you're working with differential volumes, and I'm working with
non-infinitesimal volumes for which the assumptions being made are maybe
...
pressure only \footnote{{\color{purple}Not seeing how this works now, will need to review.}
{\color{blue}I think it is in the spirit of Bear and Cheng, 5.1.31 and 5.1.32}}, 
$\rho = \rho(p)$, the change in storage can be written as:
\begin{equation}
  \frac{\partial (\rho s \phi)}{\partial t} =
  \rho s S_s \frac{\partial h}{\partial t} +
  \rho \phi \frac{\partial s}{\partial t}.
  \label{eq-sto}
\end{equation}
The specific storage \footnote{{\color{purple}It should be noted that the assumptions 
...
\end{eqnarray*}
During the assembly of equation~\ref{eq-unsat-flow}, the density $\rho$ appears 
as a factor in all the terms and cancels out. This marks the transition between 
the mass based balance law that is the starting point of the derivation, 
to a formulation that rests on a conservation of water volume as used in
MODFLOW~6. Note that as the medium approaches full saturation, 
the derivative $\partial s/ \partial t$ vanishes and $k_r$ becomes equal to 1.
In that case, the above expression converges smoothly to the saturated groundwater
flow equation used in MODFLOW~6:
\begin{equation}
...
Picard iteration method\footnote{Are we planning to support non-NR
  treatment of Richards flow?? AMP: good question - was wondering about 
  that, too} or by applying the Newton-Raphson 
