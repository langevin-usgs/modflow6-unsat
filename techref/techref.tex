...
on $h^i$ starts with the approximating relation:
\begin{equation}
  (\rho\phi)^i =
...
the discretized volume too. Substituting this in
equation~\ref{eq-delta_sto} leads to:
\begin{equation}
  \Delta \left ( \rho s \phi \right ) =
  (\rho \phi)^{i-1}\Delta s +
  \rho_{\textrm{o}} S_s s^{i,(k-1)} \Delta h.
\end{equation}
where the second term on the right-hand side is associated with
specific storage and has been linearized by evaluating the saturation
at the previous iteration level $k-1$. To convert the term containing $\Delta s$, two different approaches are available and worked out 
in detail below: a chord slope method and what is generally known as
the modified Picard method \cite{celia-1990}.
