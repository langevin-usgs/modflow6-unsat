...
This section presents the fundamental equations and theoretical framework 
governing variably saturated flow in porous media. The development begins 
with the conservation of mass for the water phase, followed by constitutive 
relations that link water content and hydraulic properties to matric potential. 
These equations form the basis for the numerical implementation in MODFLOW 6.
...
equation for the water phase has the form:
\begin{equation}
...
\end{eqnarray*}
This equation is the starting point for the analysis that follows. 
A detailed derivation can be found in, for example, Bear and Cheng, 2010.

\subsection{Storage Term}
The change in mass of water that is held in storage is given by the first 
term in equation~\ref{eq-balance-equation}. This can be expressed in terms 
of hydraulic head, the principal dependent variable for solving the system
...
or, in case that the $z$ coordinate is fixed, of hydraulic 
head: $s = s(h)$.
...
are introduced to rewrite the right-hand side of
...
\begin{eqnarray*}
  ...
  \alpha_c = \frac{1}{1 - \phi} \frac{\partial \phi}{\partial p}.
\end{eqnarray*}
Now with the approximation that the density of water depends on
pressure only, $\rho = \rho(p)$, the change in storage can be written as:
\begin{equation}
  \frac{\partial (\rho s \phi)}{\partial t} =
  ...
  \label{eq-sto}
\end{equation}
The specific storage \footnote{It should be noted that the assumptions 
underlying the specific storage formulation don't necessarily hold in the unsaturated 
case, but the error from including it is generally small; see, for example, the 
SUTRA manual.} in this equation is defined as
\begin{equation}
  S_s = \rho g \left[ \phi \beta_c + (1 - \phi) \alpha_c \right].
  \label{eq-spec-sto}
\end{equation}
Substituting this into equation~\ref{eq-balance-equation} yields
\begin{equation}
  \rho s S_s \frac{\partial h}{\partial t} +
  \rho \phi \frac{\partial s}{\partial t} +
  \nabla \cdot (\rho \mathbf{q}) -
  \rho \Gamma = 0
  \label{eq-balance-equation-sto}
\end{equation}
This expression can be further simplified when using the
following identity for the divergence term
\begin{equation}
  \nabla \cdot (\rho \mathbf{q}) = 
    \mathbf{q} \cdot (\nabla \rho) + \rho \nabla \cdot \mathbf{q}
  \label{eq-div-rule-rho}
\end{equation}
Now the storage term above has a component that depends on the
change of $\rho$ over time
\begin{equation}
   \frac{\partial (\rho s \phi)}{\partial t} = \dots + 
    s \phi \frac{\partial \rho}{\partial t}.
\end{equation} 
Then the term containing the gradient of the density in
equation~\ref{eq-div-rule-rho} can be discarded because in practice the 
temporal variation in $\rho$ is much larger than its spatial variation:
\begin{equation}
  \left| s \phi \frac{\partial \rho}{\partial t} \right| \gg
  \left| \mathbf{q} \cdot (\nabla \rho) \right|
  \label{eq-density-assumption}
\end{equation}
The balance equation for variably saturated flow then becomes:
\begin{equation}
  s S_s \frac{\partial h}{\partial t} +
  \phi \frac{\partial s}{\partial t} +
  \nabla \cdot \mathbf{q} -
  \Gamma = 0
  \label{eq-balance-equation-gradq}
\end{equation}
The fact that the density is no longer present in this
equation deserves some attention. The terms now correspond to
a volume and no longer to mass.\footnote{We should discuss the
consequences. This might also be where we can link to the
'apparent volume' viewpoint}.

\subsection{Fluid-Flux Equation}
The fluid flux $\mathbf{q}$ in equation~\ref{eq-balance-equation-gradq}
is defined by Darcy's law for unsaturated flow:
\begin{equation}
...
\begin{equation}
  ...
  \label{eq-hydr-cond}
\end{equation}
the hydraulic conductivity tensor, and the hydraulic head equals the 
sum of the pressure head $\psi = p/(\rho g)$ and the elevation head:
\begin{equation}
  h = \psi + z.
\end{equation}

The concept of relative hydraulic conductivity is commonly used to 
capture the dependence of the conductivity on saturation. For example, 
in an isotropic porous medium, the fluid flux (also specific discharge)
is given by:
\begin{equation}
...
it is assumed that relative hydraulic conductivity is isotropic. With this
simplifying assumption, equation~\ref{eq-unsat-darcy-flux} can be written as
\begin{equation}
...
pointed out that the effect of saturation on the conductivity generally 
depends on direction \cite{bear-1987}.

\subsection{Variably Saturated Flow Equation}
Using the results from previous sections, the macroscopic mass balance 
equation for the flow of water in a partially saturated porous medium, 
assuming that the air phase is stagnant and its pressure is atmospheric 
everywhere, can be written as\footnote{$s$ is a function of $\psi$ and
that should be kept clear. Will update this.}
\begin{equation}
  \left(s S_s  + C(\psi)\right)\frac{\partial h}{\partial t} =
  \nabla \cdot \left[ 
    k_\text{r}(s) \mathbf{K}_{\textrm{sat}} \cdot \nabla h \right] +
  \Gamma,
  \label{eq-balance-equation-darcy}
\end{equation}
with
\begin{eqnarray*}
  s &=& \text{water saturation} \\
  S_s &=& \text{specific storage} \\
  C &=& \text{moisture capacity} \\
  h &=& \text{hydraulic head} \\
  k_r &=& \text{relative hydraulic conductivity} \\
  \mathbf{K}_{\textrm{sat}} &=& \text{saturated hydraulic conductivity} \\
  \Gamma &=& \text{volumetric source-sink term}
\end{eqnarray*}
and using that, with elevation held constant, the time-derivative 
of saturation can be expressed in terms of the moisture capacity as
\begin{equation}
  \phi \frac{\partial s}{\partial t} = 
  \phi \frac{\partial s}{\partial \psi} \frac{\partial \psi}{\partial t} =
  C(\psi) \frac{\partial h}{\partial t},
\end{equation}
such that the storage term is now formulated in terms of $h$, the
dependent variable that will eventually be solved for. 
Equation~\ref{eq-balance-equation-darcy} constitutes the continuous
variably saturated flow equation that forms the basis for the numerical 
solution and the discretization technique that is explained in 
section~\ref{sec-num-sol} below.

{\color{cyan} A temporary note that needs to be straightened out is the 
following. The specific moisture capacity is often defined as the slope
...
\end{equation}
For example, as is done in Van Genuchten's original paper (1980). 
However, Bear\&Cheng define it (6.3.20) as
\begin{equation}
  C(p) = \phi \frac{\partial s}{\partial p}
  \label{eq-moist-cap-bear}
\end{equation}
and a conversion to pressure head follows later renaming the capacity
function to $C'$. This is what we have followed initially in this
report. Now you can argue that it doesn't matter all that
much because the change in water content $\theta$ is fully driven by 
the change in $s$ anyway. But it ties directly into our discussion about
potentially double counting the storage change due to compressibility
of the pore matrix. In our derivation above, we have that effect already
accounted for through the definition of $S_s$. So in order to keep doing 
things consistent, we would need to make sure that the parametrizations that 
are used for $C$ are actually as defined in the spirit of 
equation~\ref{eq-moist-cap-bear}? To add to all possible confusion,
the article by Miller et al. (1998) talks about $S_s$ as a coefficient
that accounts for the water compressibility only. This would naturally
coincide with having the porosity $\psi$ within the derivative again
such that changes to the pore space are accounted for through $C$
and not through $S_s$...
} % end color
...
leads to a relation between relative hydraulic conductivity and pressure 
potential \cite{???}. These three analytical functions are required for 
...
\label{sec-num-sol}
Equation~\ref{eq-balance-equation-darcy}, together with the boundary 
conditions described in section~\ref{sec-boundary-conditions}, is a 
highly non-linear partial differential equation that has no general 
...
saturated flow equation is discretized using the controle-volume
finite-difference (CVFD) method. The method replaces the continuum of
coordinates with a set of discrete points in space and time. 
The control volume is called a cell and the  point at the center of 
the cell a node. The collection of cells with their connections is the
model grid. The partial derivatives in the equation can be approximated
by differencing head values between neighbouring cells and between 
subsequent time values. The resulting finite-difference equations will 
have terms that are a function of the discretized dependent variable, 
such as $k_r = k_r(h)$ and $s = s(h)$. These non-linearities can be 
overcome using a standard Picard iteration method, or by applying 
the Newton-Raphson linearization that is known to provide more stability
when solving for cells that are under a water-table condition. 
For each cell in the the grid that is under variably saturated flow 
conditions, the linearized balance terms are added to the system 
representing the motion equations in the total (saturated and unsaturated) 
simulation domain. For every outer iteration, the linearized system is 
solved by the MODFLOW iterative solver as part.

\subsubsection*{Integration over the cell volume}
Integration of equation~\ref{eq-balance-equation-darcy} over the representative
elementary volume $V_n$ converts the equation into a water volume balance
on cell $n$. Assuming that any source or sink is distributed uniformly
throughout the cell, or that the specified rate always represents a volume
average, its total volumetric rate for cell $n$ is:
\begin{equation}
  Q_{n, \Gamma} = \int_{V_n} \Gamma \,dV = V \Gamma_n.
\end{equation}
The integral over the change in storage can be calculated as
\begin{eqnarray}
  Q_{n,\Sigma} 
    &=& \int_{V_n} \left( s(\psi) S_s  + C(\psi) \right)
      \frac{\partial h}{\partial t} \,dV \\
    &=& V_n  \left( s(\psi_n) S_s  + C(\psi_n) \right)
      \frac{\partial h_n}{\partial t},
\end{eqnarray}
where it is used that $s_n$ and $\psi_n$ are representative values for
cell $n$. Generally when $\psi$ is constant, $h$ will depend on the 
coordinate $z$. But in this case
\begin{eqnarray}
  \int_{V_n} \frac{\partial h}{\partial t} \,dV 
    &=& \int_{V_n} \frac{\partial (\psi - z)}{\partial t} \,dV \\
    &=& \int_{V_n} \frac{\partial \psi}{\partial t} \,dV - 
        \frac{\partial}{\partial t} \int_{V_n} z \,dV \\
    &=& V_n \frac{\partial \psi_n}{\partial t} - 0 \\
    &=& V_n \frac{\partial h_n}{\partial t}.
\end{eqnarray}
The time discretization of the storage term will be in 
section~\ref{sec-temp-disc-sto}. Finally, the flux term
can be manipulated, by applying Gauss' divergence theorem,
into an integral over the surface $S_n$ that encloses the 
cell volume:
\begin{equation}  
  \int_{V_n} \nabla \cdot \left[ 
    k_\text{r}(s) \mathbf{K}_{\textrm{sat}} \cdot \nabla h \right] \,dV =
      \int_{S_n} \left[ 
      k_\text{r}(s) \mathbf{K}_{\textrm{sat}} \cdot \nabla h \right] \,\cdot d\mathbf{S},
\end{equation}
such that the flow from cell $m$ into cell $n$ is given by the
integral over the area of the interface:
\begin{equation}
  Q_{n,m} = \int_{S_n \cap S_m} \left[ k_\text{r}(s) \mathbf{K}_{\textrm{sat}} 
    \cdot \nabla h \right] \cdot \,d\mathbf{S}
\end{equation} 
The spatial discretization of the Darcy flux expression will be the
subject of section~\ref{sec-flow-discretization}.

\subsubsection*{Water Balance on a Cell}
After the integration over the cell volume, the resulting equation
represents the concept of (volumetric) water balance on a cell $n$ as
\begin{equation}
...
\label{sec-temp-disc-sto}
{\color{blue}
The previous sections have been seriously revised, and the following
paragraphs need to be rewritten. At the same time, I hope to introduce
a clean way to track the change in $\rho\phi$ over time, somewhat
pending on rounding up our discussion on specific storage and the
moisture capacity function, i.e., is it $C = \phi ds/d\psi$ or 
$C = d (\phi s)/d\psi$ and how is $S_s$ represented in either case.
}
...
Therefore, instead of applying the product rule in the continuous 
domain and working with the expression on the right-hand side of 
equation~\ref{eq-sto}, discrete differences are used as a
starting point. The volumetric rate of change over a time interval 
$\Delta t$ in a representative elementary volume $V$ is given
by
\begin{equation}
  Q_{\Sigma} = \frac{ V \Delta\left ( \rho s \phi \right )}{\rho \Delta t}.
\end{equation}
The change in the mass of water stored per volume of aquifer can be
expanded as
\begin{equation}
...
are evaluated and the time interval $\Delta t$ for which the 
...
{\color{blue}
Some remarks: from here on I'd like to derive the discretized storage terms.
I am not sure though if the splitting into two distinct storage
rates (specific and 'unsaturated') is reasonable. And maybe there
is no need for it anyway... Can we define $Q_{Ss}$ as the rate 
that comes from varying $h$ exclusively? The following three
storage formulations from our notes/derivations are equivalent:
\begin{eqnarray}
  Q_\Sigma &=& \rho_0 \phi_0 \Delta s + 
    S_s \left( \psi^i s^i - \psi^{i-1} s^{i-1} \right) \\
  Q_\Sigma &=& \rho^{i-1} \phi^{i-1} \Delta s + 
    S_s s^i \Delta h \\
  Q_\Sigma &=& [\rho \phi]_{h=0} \Delta s + 
    S_s \left( h^i s^i - h^{i-1} s^{i-1} \right)
\end{eqnarray}
So is it the second equation maybe, that naturally defines the two
distinct rates.
}
...
\end{equation}
The moisture capacity function is defined in this equation by\footnote{{\color{purple}This is different from the function defined in eqn \ref{eq-C-related-to-WRC}.}}:
\begin{equation}
  C^{i,(k-1)} = 
...
{\color{purple}
\section{Comparison of different formulations of the storage term}

Expansion of the storage term using the product rule gives
\begin{equation}
\frac{\partial (\rho s \phi)}{\partial t}
  = \rho \phi\, \frac{\partial s}{\partial t}
  + \rho s\, \frac{\partial \phi}{\partial t}
  + \phi s\, \frac{\partial \rho}{\partial t} .
\label{eq-sto-expanded}
\end{equation}
The three terms on the right-hand side represent three ways to change the mass of water stored. The first term involves changing the volume of water by filling the existing pore space with more water of the same density. The second term involves changing the volume of pore space, adding or withdrawing water of the same density to maintain the same saturation. The third term involves changing the density of the water, keeping the same volume of water. The second and third terms on the right-hand side together represent specific storage.

\subsection{Formulation in terms of volumetric water content}

As an alternative, one can separate out only the density-derivative term, leaving the product $s \phi$, the volumetric water content, intact as a dependent variable:
\begin{equation}
\frac{\partial (\rho s \phi)}{\partial t}
  = \rho\,\frac{\partial (s\phi)}{\partial t}
  + \phi s\, \frac{\partial \rho}{\partial t} .
\label{eq-sto-expanded-rho}
\end{equation}
The focus of this formulation is the first term on the right-hand side, which involves the rate of change of the volumetric water content. This term accounts for both the change in saturation and the change in porosity, with the latter resulting from the compressibility of the porous matrix. The second term on the right-hand side accounts for the compressibility of water.\footnote{{\color{purple}It might also be considered to include the effects of solute concentration and temperature on density, but that's another topic altogether, and a rather deep one.}}

The first term on the right-hand side of equation \ref{eq-sto-expanded-rho} is the only one considered in Celia's work. Thus, Celia's approach is based on the assumption that the water is incompressible ($\frac{\partial \rho}{\partial t} = 0$), which makes the second term on the right-hand side vanish. However, note that if we further simplify the expression by also assuming confined conditions ($\frac{\partial s}{\partial t} = 0$), then equation \ref{eq-sto-expanded-rho} reduces to
\begin{equation}
  \frac{\partial (\rho s \phi)}{\partial t}
  = \rho s \frac{\partial \phi}{\partial t} .
\label{eq-sto-const-rho-s}
\end{equation}
This expression accounts only for the compressibility of the porous matrix. While this might be appropriate in some use cases, it is not generally what we expect in terms of accounting for compressibility effects (which we typically want to include water), i.e., specific storage. And yet, from this analysis we see that it is inherent in the Celia-type approach based on volumetric water content.

According to Farthing and Ogden (2017) [need a proper reference here], it is common to account for specific storage by simply adding it to the Celia-type storage term. However, because the Celia term already accounts for the compressibility of the porous matrix, adding the full specific storage term results in ``double counting'' the effect of porous matrix compressibility.  A proper accounting of specific storage could presumably be achieved by a ``modified Celia-type'' approach that includes the density-derivative term (the second term on the right-hand side of equation \ref{eq-sto-expanded-rho}, instead of the full specific storage term) and tracks the evolution of density with changing pressure via the water compressibility. That said, it is worth noting that adding the full specific storage term might not be so bad because in practice the Celia-type term, when related to a water retention curve measured in the laboratory, might not actually account for matrix compressibility effects found in the field.  In that case, one can pull $\phi$ out of the derivative in the Celia-type term, resulting in the saturation-based formulation we discuss next.

\subsection{Formulation in terms of saturation}

Another alternative is to separate out only the saturation-derivative term, leaving the product $\rho \phi$, the mass of water per volume of aquifer (``bulk density of water'') at full saturation, intact as a dependent variable:
\begin{equation}
\frac{\partial (\rho s \phi)}{\partial t}
  = \rho \phi\, \frac{\partial s}{\partial t}
  + s\,\frac{\partial (\rho \phi)}{\partial t}
\label{eq-sto-expanded-s}
\end{equation}
The focus of this formulation is the first term on the right-hand side, which involves the rate of change in saturation. The second term on the right-hand side accounts for the compressibility of the water and the porous matrix, i.e., specific storage.

This is the formulation we've adopted in this writeup, and it is consistent with the saturation-based formulations used in VS2D \cite{vs2d} and SUTRA \cite{vosssutra}. To conserve mass, it appears we need to either track the evolution of the specific storage term with time (see equation \ref{eq-rhophi-i}) or neglect specific storage altogether when the medium is partially saturated. Tracking specific storage should provide a ``smooth'' transition between partially and fully saturated conditions, as the full specific storage term would always be accounted for.\footnote{{\color{purple}Would we need to continue tracking specific storage explicitly even under fully saturated conditions? I think not, but I'm not entirely sure.}}.  Neglecting specific storage under partially saturated conditions would obviously cause a ``discontinuity'' in that that term would suddenly some into play under under fully saturated conditions. Not sure it makes a whole lot of difference from a model accuracy standpoint. Not tracking specific storage would certainly be simpler.

\subsection{Formulation in terms of ``mass saturation''}

The final alternative is to separate out only the porosity-derivative term, leaving the product $\rho s$, the mass of water per volume of pores (``mass saturation'' or ``pore density of water''\footnote{{\color{purple}Terms I made up; not used by anyone else as far as I know.}}), intact as a dependent variable:
\begin{equation}
\frac{\partial (\rho s \phi)}{\partial t}
  = \phi\,\frac{\partial (\rho s)}{\partial t}
  + \rho s\,\frac{\partial \phi}{\partial t}
\label{eq-sto-expanded-phi}
\end{equation}
The focus of this formulation is the first term on the right-hand side, which involves the rate of change of the ``mass saturation''. This term accounts for both the change in saturation and the change in density, with the latter resulting from the compressibility of the water. The second term on the right-hand side accounts for the compressibility of the porous matrix.

Here, one is apparently faced with a choice between either tracking the evolution of porosity or assuming it is constant, i.e., that the porous matrix is incompressible. If porosity is constant, this formulation suffers from a limitation similar to that of the Celia-like formulation. Under confined conditions ($\frac{\partial s}{\partial t} = 0$), the expression reduces to
\begin{equation}
  \frac{\partial (\rho s \phi)}{\partial t}
  = s \phi \frac{\partial \rho}{\partial t} .
\label{eq-sto-const-phi}
\end{equation}
which considers only the compressibility of the water and thus does not allow us to account fully for specific storage. Another issue with this formulation is that there probably are no parameterizations available for calculating $\rho s$ as a function of pressure or pressure head.
